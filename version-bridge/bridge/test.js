...
(async () => {
  ...
  const { payload: r1, targetEndpointUrl: r1EndpointUrl } = await bridge({ payload: discoverIn, action: "discover", targetSubscriberId: BPP_ID, registryUrl: REGISTRY_URL });
  ...
  check("target's endpointUrl returned alongside payload", r1EndpointUrl === "http://localhost:3000/api/webhook");
  ...
  const { payload: r2, targetEndpointUrl: r2EndpointUrl } = await bridge({ payload: statusOut, action: "on_status", targetSubscriberId: BAP_ID, registryUrl: REGISTRY_URL });
  ...
  check("target's endpointUrl returned alongside payload", r2EndpointUrl === "http://localhost:3000/api/bap-webhook");
  ...
  const { payload: r3 } = await bridge({ payload: alreadyMatching, action: "on_status", targetSubscriberId: BPP_ID, registryUrl: REGISTRY_URL });
